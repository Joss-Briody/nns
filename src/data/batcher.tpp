...
template <typename Dataset>
std::shared_ptr<Dataset> BatchLoader<Dataset>::getDataset() {
  return dataset;
}
...
void BatchLoader<Dataset>::shuffleTrain() {
  ...
  for (size_t i = 0; i < dataset->trainImages.size(); ++i) {
    ...
    std::swap(dataset->trainLabels[i], dataset->trainLabels[indexes[i]]);
  }
...
void printMax(Container &values) {
  for (auto &vec : values) {
    std::cout << " Max is: " +
                     std::to_string(*std::max_element(vec.begin(), vec.end()));
  }
}

template <typename T, typename V>
void printArrays(
    std::pair<std::vector<std::vector<T>>, std::vector<V>> &&values) {
  ...
  std::for_each(values.second.begin(), values.second.end(), [](const V &v) {
